...
implement put_int(b, n) = let
  fun _put_digits {fuel:nat} .<fuel>.
    ...
    if fuel <= 0 then ()
      ...
      put_byte_safe(b, n + 48)
    else let
      val () = _put_digits(b, n / 10, fuel - 1)
      val () = put_byte_safe(b, (n mod 10) + 48)
    in end
in
  if n < 0 then let
    val () = put_byte_safe(b, 45)
    val abs_n = ~n
  in
    if abs_n < 0 then
      put_byte_safe(b, 48)
...
implement bput_int(b, v) = let
  ...
  val () = (if is_neg then put_byte_safe(b, 45) else ())
  fun emit {ld:agz}{fuel:nat} .<fuel>.
    (digits: !$A.arr(byte, ld, 16), b: !builder0 >> builder0, pos: int, fuel: int fuel): void =
    if fuel <= 0 then ()
    else if pos < 0 then ()
    else let
      val () = put_byte_safe(b, byte2int0($A.get<byte>(digits, $AR.checked_idx(pos, 16))))
...
fn _check_byte {l:agz}
  (arr: !$A.arr(byte, l, BUILDER_CAP), idx: int, expected: int): bool =
  $AR.eq_int_int(byte2int0($A.get<byte>(arr, $AR.checked_idx(idx, BUILDER_CAP))), expected)
...
fn _test_put_int(): bool = let
  ...
  val @(arr, _) = to_arr(b)
  val ok = l1 = 2
    && _check_byte(arr, 0, 52)
    && _check_byte(arr, 1, 50)
  val () = $A.free<byte>(arr)
in ok end
...
fn _test_put_int_zero(): bool = let
  ...
  val () = put_int(b, 0)
  val @(arr, len) = to_arr(b)
  val ok = len = 1 && _check_byte(arr, 0, 48)
  val () = $A.free<byte>(arr)
in ok end
...
fn _test_bput(): bool = let
  ...
  val @(arr, len) = to_arr(b)
  val ok = len = 2
    && _check_byte(arr, 0, 104)
    && _check_byte(arr, 1, 105)
  val () = $A.free<byte>(arr)
in ok end
...
fn _test_bput_int(): bool = let
  ...
  val @(arr, len) = to_arr(b)
  val ok = len = 3
    && _check_byte(arr, 0, 49)
    && _check_byte(arr, 1, 50)
    && _check_byte(arr, 2, 51)
  val () = $A.free<byte>(arr)
in ok end
